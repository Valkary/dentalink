...
export default function getAllPatients(req, res) {
  return new Promise((resolve, reject) => {
    if(req.method === "POST"){
      ...
      conn.query(`SELECT ap.date, ap.start_time, ap.end_time, p.names, p.last_names, pr.name as procedimiento FROM appointments ap INNER JOIN patients p ON p.id = ap.id_patient INNER JOIN procedures pr ON pr.id = ap.id_procedure WHERE ap.id_patient = ${patient_id};`, (err, result) => {
        let appointments = [];
